...
- Load a `live/*` overlay and stream a video via RTMP to obs for live streaming with live data. Live data can also be simulated using the CIQ Simulator (Alternative to playback).
- Load a `live/*?playback=true` overlay and upload a fit file to simulate an activity. You can then sync the playback with a recorded video and record it.
